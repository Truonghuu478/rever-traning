<template>
  <form class="form-group card text-left p-3">
    <label class="font-weight-bold"
      >Bài 178: Hãy liệt kê các số chẵn trong mảng 1 chiều các số nguyên thuộcđoạn [x, y] cho trước (x, y là các số nguyên)</label >
    <div class="d-flex">
...
export default {
  ...
  methods: {
    ...
    add() {
      if (this.validate(this.n, "B178", 0)) {
    ...
    perform() {
      ...
      if ( this.validate(this.x, "B178", 0) ||this.validate(this.y, "B178", 0)) {
        let result = "";
        for (let i = 0; i < this.list.length; i++) {
          if ( this.list[i] < this.y && this.list[i] > this.x && this.list[i] % 2 === 0) {
            result += " " + i;
          }
        }
