<template>
  <form class="form-group card text-left p-3">
    <label class="font-weight-bold" >Bài 183: Hãy liệt kê các vị trí mà giá trị tại đó là giá trị lớn nhất trong mảng 1 chiều các số thực</label>
    <div class="d-flex">
  ...
  methods: {
    ...
    perform() {
      ...
      for (let i = 0; i < this.list.length; i++) {
        if (this.list[i] === this.list[max]) {
          result += " " + i;
        }
      }
      this.results = result
        ? `Danh sách vị trí có giá trị lớn nhất là :` + result
        : `Danh sách không có giá trị thoả điều kiện`;
      this.isSubmit = true;
    },
  },
};
