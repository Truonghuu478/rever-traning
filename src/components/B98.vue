...
<template>
  <form class="form-group card p-3 text-left" @submit.prevent="submit">
    <label class="font-weight-bold" >Bài 98: Lập chương trình giải hệ: ax + by = c, Dx + ey = f. Các hệ số nhập từ bàn phím </label>
    <div class="d-flex ">
      <div class="form-group w-50">
        <label class="form-label">Nhập cạnh a :</label>
        <input v-model="a" class="form-control" placeholder="Nhập a :"/>
      </div>
      <div class="form-group w-50  text-left">
        <label class="form-label">Nhập cạnh b :</label>
        <input v-model="b" class="form-control" placeholder="Nhập b :"/>
      </div>
      <div class="form-group w-50  text-left">
        <label class="form-label">Nhập cạnh c :</label>
        <input v-model="c" class="form-control" placeholder="Nhập c :"/>
      </div>
    </div>
  ...
  methods: {
    submit() {
      ...
      if (isNaN(a) || isNaN(b) || isNaN(c)) {
        alert("B97 : Vui lòng nhập a,b,c bằng số !")
      } else {
        let results = "Đây là tam giác"
        if (a === b && b === c) {
          results += " đều "
        } else {
