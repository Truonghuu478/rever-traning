<template>
  <form class="form-group card text-left p-3">
    <label class="font-weight-bold">Bài 205: Tính tổng các giá trị lớn hơn trị tuyệt đối của giá trị đứng liền sau nó trong mảng 1 chiều các số thực</label>
    <div class="d-flex">
  ...
  methods: {
    ...
    perform() {
      ...
      for (let i = 0; i < this.list.length; i++) {
          let nextNumber = this.list[i-1] > 0 ?  this.list[i-1] : this.list[i-1]* -1
          if(this.list[i] > nextNumber) {
            s += this.list[i]
          }
      }
      this.results = "Tổng các giá trị lớn hơn giá trị đứng liền trước là : "+ s
      this.isSubmit = true;
    },
  },
};
