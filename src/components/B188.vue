<template>
  <form class="form-group card text-left p-3">
    <label class="font-weight-bold">Bài 188: Hãy liệt kê các vị trí chẵn lớn nhất trong mảng 1 chiều các số nguyên</label>
    <div class="d-flex">
...
export default {
  ...
  methods: {
    ...
    add() {
      if (this.validate(this.n, "B188",0)) {
    ...
    perform() {
      ...
      for (let i = 0; i < this.list.length; i++) {
        if (this.list[i] % 2 === 0) {
          if (!this.list[evenMaxIdx] || this.list[i] < this.list[evenMaxIdx]) {
            evenMaxIdx = i;
          }
        }
      }
      ...
      if (evenMaxIdx !== -1) {
        for (let i = 0; i < this.list.length; i++) {
          if (this.list[i] === this.list[evenMaxIdx]) {
            results += " " + i;
          }
        }
        this.results = `Mảng có các vị trí bằng giá trị chẵn   ${this.list[evenMaxIdx]} lớn nhất là : ${results}`
      } else {
        this.results = "Mảng không có giá trị chẵn";
      }
