...
<template>
  <form class="form-group card  text-left p-3">
    <label class="font-weight-bold">Bài 148: Tìm số nguyên tố cuối cùng trong mảng 1 chiều các số nguyên. Nếu mảng không có số nguyên tố thì trả về  -1</label>
    <div class="d-flex ">
  ...
  methods: {
    ...
    add() {
      if (isNaN(this.n) || this.n === null || parseInt(this.n) !== +this.n) {
        alert("B148 : Vui lòng nhập thông tin n bằng số nguyên")
      } else {
    ...
    perform() {
      ...
      this.results = result !== -1 ? "Số nguyên tố cuối cùng trong mảng là : " + this.list[result] : " Mảng không có số nguyên tố"
      this.isSubmit = true
    }
  }
}
