<template>
  <form class="form-group card text-left p-3">
    <label class="font-weight-bold">Bài 182: Cho mảng 1 chiều các số thực. Hãy viết hàm liệt kê tất cả các giá trị trong mảng có ít nhất 1 lận cận trái dấu với nó</label>
    <div class="d-flex">
...
export default {
  ...
  methods: {
    ...
    add() {
      if (this.validate(this.n, "B182", 0)) {
    ...
    perform() {
      if (!this.list.length) {
        ...
        for (let i = 0; i < this.list.length; i++) {
            let valueAfter = !isNaN(this.list[i-1]) ? this.list[i-1] : 0
            let valueBefore = !isNaN(this.list[i+1]) ? this.list[i+1] : 0
          if ((valueAfter < 0 && this.list[i] >0) ||  (valueBefore < 0 && this.list[i] > 0) || (valueAfter > 0 && this.list[i] < 0) ||  (valueBefore > 0 && this.list[i] < 0 )) {
            result += " " + i;
          }
        }
