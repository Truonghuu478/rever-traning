...
<template>
  <form class="form-group card  text-left p-3">
    <label class="font-weight-bold">Bài 156: Hãy tìm giá trị trong mảng các số thực gần giá trị x nhất</label>
    <div class="d-flex ">
    ...
    perform() {
      if (this.list.length && this.validate(this.x)){
        ...
        for (let i = 1; i < this.list.length; i++) {
          let a = this.list[i] >  this.x ?this.list[i] -  this.x : this.x - this.list[i]
          let b =  this.list[result] >  this.x ?this.list[result] -  this.x : this.x - this.list[result]
          if(a < b){
            result = i
          }
        }
        this.results =  "Giá trị trong mảng xa giá trị x gần nhất là :" + this.list[result]
        this.isSubmit = true
      }else this.results = ""
